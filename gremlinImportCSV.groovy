...
csvDir = "/tmp/export"
confDir = 'conf/neo4j-server.properties'

maxInsertsBeforeCommit = 100000

println('cleaning old database')
g = Neo4jFactory.open(confDir)

g.shutdown()
Neo4jCleanup.clear(g)
...
/*-------------------------------------- NEO4J VERTICES CREATION FROM CSV FILES -------------------------------------*/
println 'importing nodes...'

counter = 0

// Parse db_dbnode entries stored in nodes.csv, creates a node vertex identified by id and add it to Neo4jGraph
...
// and add it to Neo4jGraph. Switch case ensure the correct type for the value
...
// Parse db_dbextra entries stored in extras.csv, creates an extra vertex identified by id and add it to Neo4jGraph.
...
// Parse db_dbcomment entries stored in comments.csv, creates a comment vertex identified by id and add it to Neo4jGraph
...
// Parse db_dbuser entries stored in users.csv, creates a user vertex identified by id and add it to Neo4jGraph
...
/*--------------------------------------- NEO4J EDGES CREATION FROM CSV FILES ---------------------------------------*/
